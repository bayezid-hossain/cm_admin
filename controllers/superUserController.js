...
exports.approveBus = catchAsyncErrors(async (req, res, next) => {
  ...
  bus.status = req.body.approvalStatus;
  await bus.save({ validateBeforeSave: false });

  profiler.done({
    message: `Bus ${bus.busName} with ID: ${req.params.id} was ${req.body.approvalStatus}`,
...
exports.getBus = catchAsyncErrors(async (req, res, next) => {
  const profiler = logger.startTimer();
  var populateQuery = [
    { path: 'owner', select: 'companyName' },
    { path: 'routeId', select: 'name' },
  ];

  const bus = await Bus.findById(req.params.id).populate(populateQuery);
  if (!bus) {
    return next(
      new ErrorHandler(`No route found with the id : ${req.params.id}`, 400)
    );
